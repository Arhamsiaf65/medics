...
const statusMessages: Record<string, { subject: string; heading: string; color: string }> = {
    approved: {
...
export const startEmailWorker = (): Worker => {
    const worker = new Worker<AppointmentEmailPayload>(
        "emailQueue",
        async (job: Job<AppointmentEmailPayload>) => {
